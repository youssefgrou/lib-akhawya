...
$req=$bdd->query("SELECT * from student_int");
$sql = "SELECT * FROM student_int";
  ...
  if(empty($name) || empty($phone) || empty($bookname))
{
  $msg= "All field are required";
    ...
    $insert="INSERT INTO `student_int`(`name`,`phone`,`cin`,`ref`,`bookname`,`issueddate`,`expirydate`,`section`) 
...
if(isset($_GET['deleteid'])){
    $id=$_GET['deleteid'];
    $sql="DELETE FROM student_int WHERE id='$id'";
...
if(isset($_POST['save-btn']) && isset($_POST['expired'])) {
    ...
    $stmt = $bdd->prepare("UPDATE student_int SET validation = 1 WHERE id = ?");
...
<section class="mt-3" id="header" align="center">
...
<tbody>
    ...
    if (!$expiredRowsExist) { // Display a message if no expired rows found
        echo '<tr class="text-center"><td colspan="3">No expired rows found</td></tr>';
    }
    ?>
</tbody>
...
<tbody id="tbody">
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            <a href="editStudent.php?updateid='.$row["id"].'" class="btn btn-success" name="edit">
                <i class="fa fa-edit"></i> 
            </a>

            <a href="student-intern.php?deleteid='.$row["id"].'" class=" btn btn-danger m-0" name="b_id"
